...
public class PanelAtenderPacientes extends javax.swing.JPanel {
    ...
    private void eventoTabla(int idDoctor){
        ...
        TableActionEvent event = new TableActionEvent() {
            ...
            public void Action(int row, String texto) {
                Cita citaTemp = (Cita)TablaAtencion.getValueAt(row, 0);
                citaTemp.getDoctorSolicitado().setIdDoctor(idDoctor);
                JDialogAtencion a = new JDialogAtencion((JFrame)SwingUtilities.getWindowAncestor(PanelAtenderPacientes.this), true, citaTemp, null);
                a.setLocationRelativeTo(null);
                a.setVisible(true); 
                try {
                    citasPendientes = new ControladorCita().obtenerCitasPendientesPorDoctor(idDoctor);
                    actualizarTabla();
                } catch (Exception e) {
    ...
    private void cargarDatos(int idDoctor) throws Exception {
        ...
        citasPendientes = controladorCita.obtenerCitasPendientesPorDoctor(idDoctor);
        actualizarTabla();
    ...
    private void actualizarTabla() {
        ...
        for (Cita cita : citasPendientes) {
            model.addRow(new Object[]{ cita, cita.getPacienteSolicitante().getNombre()});
        }
    }
